...
// @version      1.0
// @description  I. 删除提交时的确认提示; II. 添加"重复上次提交"按钮，因延迟提交失败时可以直接重复上次提交的内容; III. 弹出选老师窗口中添加"快速选择"按钮，一键选择+确定; IV. 选择选课页面后自动点击检索按钮
// @author       ZZJ
// @match        *://202.202.1.41/*
...
function clearResubmitStorage() {
    localStorage.removeItem(Last_Submit_Table_Storage_Key['xk']);
    localStorage.removeItem(Last_Submit_Table_Storage_Key['yy']);
    localStorage.removeItem(Last_Submit_Table_Storage_Key['fx']);
    localStorage.removeItem(Last_Submit_Table_Storage_Key['ts']);
    localStorage.removeItem(Last_Submit_DOM_Storage_Key['xk']);
    localStorage.removeItem(Last_Submit_DOM_Storage_Key['yy']);
    localStorage.removeItem(Last_Submit_DOM_Storage_Key['fx']);
    localStorage.removeItem(Last_Submit_DOM_Storage_Key['ts']);
}

(function() {
    // if (name = 'frmMain') {
    //     console.log(HEADER + this.location.pathname);
    // }
    // if (name == 'frmRpt'){
    //     console.log(HEADER + this.location.pathname);
    // }
    ...
    if (Delete_Submit_Prompt && name == 'frmRpt'){
        ...
        if (tpe) {
            ...
            if (document.scripts[tpe].text.search("是否提交记录")==-1){
                console.log(HEADER + "已删除提交确认框！");
            }else{
                console.error(HEADER + "未删除提交确认框！");
            }    
        }
    }
    //* -----------------------------------------------------------------
    ...
    if (Append_Resubmit_Button && name == 'frmMain'){
        ...
        if (leixing) {
            ...
            if (subBtn != null) {
                ...
                //* 新按钮的css
                var resubBtnCss = document.createElement("style");
                resubBtnCss.setAttribute("type", "text/css");
                resubBtnCss.innerHTML= BTN_CSS; // 元素的类为
                document.body.appendChild(resubBtnCss);
                ...
                console.log(HEADER + "已添加重复上次提交" + leixing + "按钮。");
                ...
                resubBtn.onclick = function(subBtn){
                    return function(){
                        var val = localStorage.getItem(Last_Submit_Table_Storage_Key[leixing]);
                        if (val != null) {
                            //* 读取 oTable:
                            ...
                            var tmp = JSON.parse(localStorage.getItem(Last_Submit_DOM_Storage_Key[leixing]));
                            ...
                            console.log(HEADER + "成功读取" + leixing + "_信息。");
                            
                            subBtn.onclick();
                        }else{
                            console.error(HEADER + leixing + "_信息不存在！");
                        }
                    };
                }(subBtn);
                ...
                console.error(HEADER + "未能添加重复上次提交按钮。");
            }
        }
            
        
    }
    //* II.3. 改写提交按钮的逻辑，把提交内容存入localStorage，并post到其他hosts的相同pathname下 -----------------
    //* 提交按钮按下时先在 localStorage 存下 frmMain(/wsxk/stu_btx.aspx) -> frmRpt(/wsxk/stu_btx_rpt.aspx) -> id=oTable 的innerHTML。点击重复提交时先从storage里调取覆盖，再执行提交按钮对应的onclick。
            ...
            if (subBtn != null) {
                ...
                subBtn.onclick = function(oTable, leixing){
                    return function(){
                        ...
                        localStorage.setItem(Last_Submit_DOM_Storage_Key[leixing], 
                            JSON.stringify(tmp));
            ...
            if (tr.lastElementChild.firstElementChild.tagName.toUpperCase() == "INPUT") {
                ...
                inp.onclick = function(rad, sureBtn){
                    return function(){
                        ...
                        console.log(HEADER + "快速选择了" + rad.id);
                    }
                }(rad, sureBtn);
                
            }
        }
        console.log(HEADER + "添加了快选按钮");
    ...
    if (Auto_Click_Search && name == 'frmMain'){
        ...
        if (leixing) {
            ...
            for (const btn of document.querySelectorAll(".button")) {
                if (btn.value == "检索"){
                    searchBtn = btn;
                    console.log(HEADER + "已获取检索按钮");
                    break;
                }
            }
            if (searchBtn){
                searchBtn.click(); // 模拟点击按钮
                console.log(HEADER + "已点击检索按钮");
            }else{
                console.error(HEADER + "未找到检索按钮");
            }
        }
    }
    //* --------------------------------------------------------------
